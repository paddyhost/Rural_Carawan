package com.hatchers.ruralcaravane.activity.apihelper;
...
/**
 * Created by Ashwin on 22-Jan-18.
 */
...
public class Web_SyncApiHelper
{
    public static void addNewCustomerToServer(final Activity activity, final SweetAlertDialog sweetAlertDialog,final CustomerTable customerTable)
    {
        ...
        else
        {
            ...
            if (kitchenDetailsData != null) {
                if(kitchenDetailsData.getUpload_statusValue().equalsIgnoreCase("0"))
                {
                    addKitchenToServer(activity,sweetAlertDialog,kitchenDetailsData);
                }
                else if(kitchenDetailsData.getUpload_statusValue().equalsIgnoreCase("1"))
                {
                    sweetAlertDialog.changeAlertType(SweetAlertDialog.ERROR_TYPE);
                ...
                else if(kitchenDetailsData.getUpload_statusValue().equalsIgnoreCase("2"))
                {
                    updateKitchenToServer(activity,sweetAlertDialog,kitchenDetailsData);
                }
                else if(kitchenDetailsData.getUpload_statusValue().equalsIgnoreCase("3"))
                {
    ...
    private static void addKitchenToServer(final Activity activity, final SweetAlertDialog sweetAlertDialog, final KitchenTable kitchenTable)
    {
    ...
    private static void updateKitchenToServer(final Activity activity, final SweetAlertDialog sweetAlertDialog, final KitchenTable kitchenTable)
    {
...
//http://www.hatchers.in/caravan/index.php/api/V1/addKitchenSuitability?format=json&kitchen_id=180120052626126&customerid=180120052514114&stime=2018-01-20%2017:27:32&endtime=2018-01-20%2017:27:43&state=C&mobile=9975294782&password=user@123
    ...
    private static void uploadPaymentDataToServer(final Activity activity, final SweetAlertDialog sweetAlertDialog)
    {
        final PaymentTable paymentTable= PaymentDetailsHelper.getUnUploadPaymentData(activity);
        if(paymentTable==null)
        {
            PaymentTable paymentTable1 = PaymentDetailsHelper.getPaymentDetailsData(activity);
            if(paymentTable1.getUpload_statusValue().equalsIgnoreCase("0"))
            {
                sweetAlertDialog.changeAlertType(SweetAlertDialog.ERROR_TYPE);
            ...
            else if(paymentTable1.getUpload_statusValue().equalsIgnoreCase("1"))
            {
                sweetAlertDialog.changeAlertType(SweetAlertDialog.SUCCESS_TYPE);
                sweetAlertDialog.setTitleText("Payment Successfully Uploaded");
                sweetAlertDialog.setContentText("Chulha process completed.");
                sweetAlertDialog.setConfirmText("Ok");
                sweetAlertDialog.setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                    @Override
                    public void onClick(SweetAlertDialog sweetAlertDialog) {
                    ...
                    try {
                        ...
                        if (responce.getString("status").equalsIgnoreCase("success")) {
                            if (responce.getString("message").equalsIgnoreCase("Payment Details added successfully")) {
                                ...
                                paymentTable.setUpload_statusValue("1");
